...
class FitMethod3:
    ...
    def __init__(self, x, z_l, z_r):
        ...
        fit_varnames  = list(self.ComptonPDF.__code__.co_varnames)[3:self.ComptonPDF.__code__.co_argcount]+['N','DN','sx','sy','k','eps', 'ex','ey', 'psum', "substr"]
        ...
        self.idxsubstr=fit_varnames.index('substr')
        ...
        set_par('psum'  , 0.0 , 0.1 , [0.   , 1e3]   , True)
        set_par('substr'  , 0.0 , 0.1 , [0.   , 1e3]   , False)
    ...
    def __call__(self, *par):  
        ...
        substr = par[self.idxsubstr]

        self.fit_left  = self.ComptonPDF(xx, yy, E, L, P,  V,  Q, beta) + substr
        self.fit_right = self.ComptonPDF(xx, yy, E, L, P, -V, -Q, beta) + substr
        ...
        fB = fD0 * np.conj(fC0) / ( np.abs(fC0*fC0) + k * np.sum(np.abs(fC0*fC0)) )
        ...
        ##функцифя B входит только с одной стороны
        #self.data_diff = D1
        #self.fit_diff = CB1
        #self.data_diff_error = np.sqrt( self.data_left/NL + self.data_right/NR)
        ...
        #Если p != то добавляем к хи квадрат вклад от неполяризованной части
        p = par[self.idxpsum]
        if np.abs(p) < 1e-9:
            chi2_sum = 0.0
        else:
        ...
        chi2 = chi2_diff + chi2_sum*p + chi2_L
